...
FullData_allcycles$URXUCR_final <- coalesce(
  FullData_allcycles$URXUCR, 
  FullData_allcycles$URXUCR.x, 
  FullData_allcycles$URXUCR.y
)

summary(FullData_allcycles$URXUCR_final, useNA="ifany")
...
FullData_allcycles<-FullData_allcycles %>%
  mutate(SAMPLE= ifelse(is.na(EDUCAT)| is.na(PIRCAT) | is.na(SEX)| is.na(RACEETH) | is.na(AGEGROUP)| is.na(FOODSEC) | is.na(cycleyr),0,1))
...
table(FullData_allcycles2$SAMPLE2) #10097 final sample size 
...
FullData_allcycles <- FullData_allcycles %>%
  mutate(SURVWEIGHT = case_when(
    cycleyr >= 1 & !is.na(WTSB2YR) ~ WTSB2YR * (1 / 7),
    cycleyr >= 2 & !is.na(WTSB2YR) ~ WTSB2YR * (1 / 7),
    cycleyr >= 3 & !is.na(WTSB2YR) ~ WTSB2YR * (1 / 7),
    cycleyr >= 4 & !is.na(WTSA2YR) ~ WTSA2YR * (1 / 7),
    cycleyr >= 5 & !is.na(WTSB2YR) ~ WTSB2YR * (1 / 7),
